...
## Entry 4 – [5/13/26, 6:38 PM]: Part 4 Code Implementation Wrap Up + Part 5 Completion &# Inconsistency
...
## Entry 6 – [5/14/26, 12:35 AM]: Post-Implementation Reflection
If I had more time, the main thing I would improve is my greedy counterexample. It works, but it’s not as clean or convincing as it could be, especially after realizing late that the graph is directed. I think a better version would explicitly use one-way edges to force bad decisions, instead of relying on backtracking like I did. That would make the failure of greedy much clearer and more aligned with the actual problem setup. On the implementation side, I would spend more time refining the pruning logic. Right now it works and correctly avoids exploring obviously worse paths, but the lower bound is pretty basic. A tighter bound (maybe something like estimating remaining cost using nearest relic distances) could cut down the search space a lot more and make the algorithm feel more “intentional” instead of just brute-force with pruning.

As far as extensions, I actually think this problem could be turned into a pretty cool game-like system. You could add things like trap rooms (extra cost nodes), limited resources, or even something like a moving enemy that forces a time constraint. That would turn this from just finding the cheapest path into balancing multiple constraints, which would make the search problem a lot more interesting.

Overall though, I’m happy with how it turned out. The biggest takeaway for me was how important the order of decisions is in problems like this, and how quickly greedy approaches fall apart when future consequences matter.

---

## Final Entry – [5/14/26, 12:51 AM]: Time Estimate


| Part | Estimated Hours |
|---|---|
| Part 1: Problem Analysis | 1.0 |
| Part 2: Precomputation Design | 2.0 |
| Part 3: Algorithm Correctness | 1.5 |
| Part 4: Search Design | 2.5 |
| Part 5: State and Search Space | 2.5 |
| Part 6: Pruning | 2.0 |
| Part 7: Implementation | 5.0 |
